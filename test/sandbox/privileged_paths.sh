#!/usr/bin/env bash
# Runs as root, which is the only way to test the privileged paths at all. The feature drops sudo
# for the remote user, so a test that runs as that user cannot drive a sweep, restart the
# entrypoint, or stop the daemon. Everything here needs one of those, so it cannot live in test.sh.
#
# root is not a claim about the boundary -- test.sh makes that claim, as the remote user. root is
# only the vantage point that still holds the privilege these mechanism checks need to set up.
set -e
...
check "the sweeper can be stopped, as these checks require" bash -c '
    pkill -f "sandbox\.sh daemon" 2>/dev/null || true
...
check "a socket listed as a mount is refused by the sweep, not sealed" bash -c '
    ...
    printf "%s\n" "99 98 0:99 / /tmp/vscode-ssh-auth-pretend-mount.sock rw,relatime - overlay overlay rw" \
        > /tmp/fake-mountinfo
    out=$(env SANDBOX_MOUNTINFO=/tmp/fake-mountinfo \
...
check "the same socket is sealed when it is not a mount" bash -c '
    : > /tmp/empty-mountinfo
    env SANDBOX_MOUNTINFO=/tmp/empty-mountinfo \
...
check "the warning names the host-side setting, the only real fix for wayland" bash -c '
    ...
    printf "%s\n" "99 98 0:99 / /tmp/vscode-ssh-auth-pretend-mount2.sock rw,relatime - overlay overlay rw" \
        > /tmp/fake-mountinfo2
    env SANDBOX_MOUNTINFO=/tmp/fake-mountinfo2 \
...
check "the entrypoint restarts the sweeper" bash -c '
    /usr/local/share/devcontainer/sandbox/entrypoint.sh
    sleep 1
...
check "a root-owned gpg directory is handed back at container start" bash -c '
    ...
    chown root:root "$HOME/.gnupg" && chmod 0555 "$HOME/.gnupg"
    ls -ld "$HOME/.gnupg"
    /usr/local/share/devcontainer/sandbox/sandbox.sh repair
    ls -ld "$HOME/.gnupg"
...
check "a directory the feature does not own is left alone" bash -c '
    mkdir -p /opt/not-ours && chown root:root /opt/not-ours && chmod 0555 /opt/not-ours
    /usr/local/share/devcontainer/sandbox/sandbox.sh repair
