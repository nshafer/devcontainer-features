...
# guard itself is covered with a synthetic mount table in the privileged_paths scenario.
...
mkdir -p /tmp/fake-x11
touch /tmp/fake-x11/X0

CAN_MOUNT=yes
if ! mount --bind /tmp/pretend-host-wayland.sock /tmp/fake-x11/X0 2>/tmp/mount-error; then
    CAN_MOUNT=no
fi
...
if [ "$CAN_MOUNT" = no ]; then
    ...
    echo ""
    echo "  The guard this would confirm is still tested in the privileged_paths scenario:"
    ...
    check "the feature is installed even though the mount checks were skipped" bash -c '
        ...
        # Runs as root here, so sandbox-status reports sudo reachable and exits non-zero. This is a
        # smoke check that the feature installed, so tolerate that exit code.
        sandbox-status || true'
...
check "chmod on the mount point does write through to the source" bash -c '
    cp -a /tmp/pretend-host-wayland.sock /tmp/writethrough-canary.sock
    mkdir -p /tmp/canary-dir && touch /tmp/canary-dir/X0
    mount --bind /tmp/writethrough-canary.sock /tmp/canary-dir/X0
    before=$(stat -c %a /tmp/writethrough-canary.sock); echo "source before: $before"
    chmod 000 /tmp/canary-dir/X0
    after=$(stat -c %a /tmp/writethrough-canary.sock); echo "source after:  $after"
    umount /tmp/canary-dir/X0
...
check "the sweeper refuses the bind mount and leaves the source untouched" bash -c '
    ...
    out=$(env SANDBOX_X11_DIR=/tmp/fake-x11 /usr/local/share/devcontainer/sandbox/sandbox.sh block-fixed 2>&1)
    echo "$out"
...
check "a full sweep also leaves it alone" bash -c '
    before=$(stat -c %a:%U /tmp/pretend-host-wayland.sock)
    env SANDBOX_X11_DIR=/tmp/fake-x11 /usr/local/share/devcontainer/sandbox/sandbox.sh sweep >/dev/null 2>&1
    after=$(stat -c %a:%U /tmp/pretend-host-wayland.sock)
    echo "$before -> $after"
...
check "the report surfaces a mounted wayland socket as reachable" bash -c '
    mkdir -p /tmp/wl && touch /tmp/wl/vscode-wayland-abc.sock
    mount --bind /tmp/pretend-host-wayland.sock /tmp/wl/vscode-wayland-abc.sock
    out=$(sandbox-status || true)
    echo "$out"
    umount /tmp/wl/vscode-wayland-abc.sock
