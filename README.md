...
A committed `devcontainer.json` describes the project, and your local tools do not belong in the
project. Nobody else on the repo asked to have their `sudo` removed or most of the internet blocked.
[`devc`](https://github.com/nshafer/devc) keeps these features out of the committed config. It
merges a gitignored `.devcontainer/devcontainer.local.json` over the committed file, then runs the
devcontainer CLI with the result. Its README covers the install, the merge rules and compose projects.
