...
Keeps /home and the VS Code server on Docker named volumes, so both survive a container rebuild. The server has its own volume, so you can delete it alone.
