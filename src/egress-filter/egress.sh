...
write_proxy_conf() {
    ...
    # A second address, and only when there is an inner daemon to need it. tinyproxy binds at
    # start and the container's own address exists by then, so this needs no restart later -- and
    # the bridge address does not, because dockerd has not created it yet.
    #
    # The private ranges are allowed rather than the bridge subnet, for the same reason: the subnet
    # is not known here. The exposure that opens is closed in the firewall instead, where an INPUT
    # rule drops this port on the way in from the outer network. See apply_input_guard.
    local extra_listen="" extra_allow="" ip
    ip="$(primary_ip)"
    if docker_present && [ -n "$ip" ]; then
        extra_listen="Listen $ip"
...
# What inner containers are told to use. Empty when there is no inner daemon or no usable address,
# and every caller reads that as "leave Docker alone".
docker_proxy_url() {
    local ip
...
merge_json_conf() {
    ...
    if command -v python3 >/dev/null 2>&1; then
        EGRESS_JSON_FILE="$file" EGRESS_PROXY_URL="$url" EGRESS_NO_PROXY="$np" \
            python3 -c "$prog" && return 0
...
#
# Written by `up`, which runs from this feature's entrypoint. That entrypoint runs before
# docker-init.sh, so the file is in place before dockerd first reads it. A daemon that somehow
# started earlier needs a restart to pick this up.
docker_daemon_conf() {
    docker_present || return 0
...
PY
    ...
    log "inner docker daemon proxied via $url"
}
