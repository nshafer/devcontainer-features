...
# 172.17.0.0/16 -> ^172\.17\.[0-9]{1,3}\.[0-9]{1,3}$ and so on, for any prefix length. A partial
# octet becomes an alternation of every value in its range, which is at most 128 entries and needs no
# range-to-regex cleverness. Digits and dots only, anchored at both ends, so nothing that is not an
# address literal in that subnet can match.
cidr_to_regex() {
    local cidr="$1" ip prefix a b c d o i bits span base hi v alt re="^"
    ip="${cidr%/*}"; prefix="${cidr#*/}"
    ...
    for i in 0 1 2 3; do
        ...
        if [ "$bits" -ge 8 ]; then
            ...
            # The block this octet's host bits span, as arithmetic rather than a shift and a mask.
            # Same result, and no << for an editor to mistake for a heredoc.
            span=$(( 2 ** (8 - bits) ))
            base=$(( o - o % span ))
            hi=$(( base + span - 1 ))
            alt=""
            for (( v = base; v <= hi; v++ )); do alt="${alt:+$alt|}$v"; done
            re="$re($alt)"
        fi
    done
    echo "$re\$"
...
build_list() {
    ...
    # The subnets the firewall opens, as address patterns, so the proxy forwards to them as well.
    # Nothing is widened by this: every process here can reach these peers directly already. What
    # it buys is a NO_PROXY that never has to name a subnet -- a client that sends a request for a
    # peer to the proxy gets the peer, not a 403 -- and so a containerEnv block that is the same on
    # every machine. See the README, "Sibling containers, and docker-compose".
    local lnets="" lnet
    while IFS= read -r lnet; do
        [ -n "$lnet" ] || continue
        cidr_to_regex "$lnet" >> "$tmp"
        lnets="${lnets:+$lnets }$lnet"
    done <<< "$(local_networks 2>/dev/null)"
    if [ -n "$lnets" ]; then
        {
            echo ""
            echo "# ============================================================================"
            echo "# local: subnets this container is attached to -- reachable directly, and through"
            echo "# the proxy by address, so NO_PROXY does not have to name them"
            echo "# ============================================================================"
            for lnet in $lnets; do echo "$lnet -> $(cidr_to_regex "$lnet")"; done | sed 's/^/# /'
        } >> "$ALLOWLIST_FILE"
        echo "local:    $lnets (by address, through the proxy as well)" >> "$SOURCES_FILE"
    fi
...
# Static on purpose, and the same on every machine. No subnet is in it: the proxy forwards to the
# local subnets by address (see build_list), so a client that sends a peer's address to the proxy
# gets the peer. What a client cannot get that way is a peer by *name* -- "db", "redis" -- because a
# name is not an address pattern and nothing here can know what compose called the service. Those
# come from the noProxy option, and they are the only project-specific part of the list.
#
# Static is what makes the containerEnv block in the project's devcontainer.json possible at all:
# that block cannot read the routing table, and a list that changed per machine could never match it.
#
# One function, four consumers: the shell environment, /etc/docker/daemon.json, the client config
# that docker copies into every container it starts, and the block status prints for you to paste.
no_proxy_list() {
...
# entry of ours that the block does not carry is a fault. In practice that is a name from the
# noProxy option -- "db", "redis" -- which the block has to repeat, because a client that reads the
# block sends a request for that peer to the proxy, which has no address pattern for a name and
# denies it. A peer that works from a terminal and fails from a VS Code extension.
#
# $1 is the container environment's NO_PROXY.
no_proxy_missing() {
    local have="$1" entry out=""
    [ "$have" = none ] || [ -z "$have" ] && return 0
    for entry in $(no_proxy_list | tr ',' ' '); do
        case ",$have," in *",$entry,"*) ;; *) out="${out:+$out }$entry" ;; esac
    done
    echo "$out"
}
...
status() {
    ...
    if [ -n "$nets" ]; then
        printf '  %-14s %s (direct, and through the proxy by address)\n' "local" "$nets"
    else
        printf '  %-14s none -- peers on the docker network are blocked too\n' "local"
    fi

    # Root reads PID 1 directly, and everyone else reads lines 6 and 7 of the state file. An empty line 6 is a state file from an
...
watch_daemon() {
    ...
    while true; do
        ...
        if [ "$topo" != "$last_topo" ]; then
            ...
            apply_firewall
            # The proxy allows the same subnets by address, so it has to learn the new set too.
            build_list >/dev/null 2>&1
            reload_proxy >/dev/null 2>&1
